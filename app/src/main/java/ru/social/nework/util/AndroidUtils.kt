...
object AndroidUtils {
    ...
    fun dateRangeToText(dateStartStr: String, dateEndStr: String?): String {
        val dateStartRes = dateUTCToShortDateText(dateStartStr)
        val dateEndRes =
            if (dateEndStr.isNullOrBlank()) "настоящее время" else dateUTCToShortDateText(dateEndStr)
        return ("$dateStartRes - $dateEndRes")
    }

    private fun dateUTCToShortDateText(dateStr: String): String {
        val date = ZonedDateTime.parse(dateStr).withZoneSameInstant(ZoneId.systemDefault())
        return date.format(DateTimeFormatter.ofPattern("dd MMM yyyy", Locale("ru")))
    }
